...
import numpy as np
import os
from models.transreid.utils.reranking import re_ranking
...
def eval_func(distmat, q_pids, g_pids, q_camids, g_camids, max_rank=50, is_pridect=False):
    ...
    # indices = np.argsort(distmat, axis=0)
    if is_pridect:
        # return indices[:, 0]
        return indices[:, 0:5]
...
class R1_mAP_eval():
    ...
    def delete(self):
        self.feats = self.feats[:self.num_gallery]
        self.pids = self.pids[:self.num_gallery]
        self.camids = self.camids[:self.num_gallery]
